...
export async function createUser(email: string, passwordHash: string): Promise<UserRecord> {
  ...
  database.run('INSERT INTO users (email, password_hash, created_at) VALUES (?, ?, ?)', [
    normalizedEmail,
    passwordHash,
    createdAt
  ]);
  ...
  return { id, email: normalizedEmail, passwordHash, createdAt };
}
...
export async function findUserByEmail(email: string): Promise<UserRecord | null> {
  ...
  const [id, userEmail, passwordHash, createdAt] = result[0].values[0];
  return {
    id: Number(id),
...
export async function findUserById(id: number): Promise<UserRecord | null> {
  ...
  const [userId, userEmail, passwordHash, createdAt] = result[0].values[0];
  return {
    id: Number(userId),
    email: String(userEmail),
    passwordHash: String(passwordHash),
